...
const { printWithPager, prefersAsciiOutput, detectPager, isInteractive } = require('../lib/pager');
...
class STRSQLSession {
  ...
  async _printResult(text) {
    if (!this.rl) {
      await printWithPager(text);
      return;
    }
  ...
  _tableOpts() {
    ...
    // If a pager is active, keep full values and let less handle horizontal scroll.
    // Without pager, keep compact output to avoid unreadable wrapping in raw terminals.
    const canPage = process.env.STRSQL_NO_PAGER !== '1' && isInteractive() && Boolean(detectPager());
    const autoWidth = canPage ? Number.MAX_SAFE_INTEGER : 40;
  ...
  async _executeSQL(sql) {
    ...
    try {
      if (isSelect) {
        ...
        const formatted = formatTable(result, this._tableOpts());
        await this._printResult(formatted);
        // ────────────────────────────────────────────────────────────────────
      } else {
  ...
  async _metaCommand(cmd) {
    ...
    switch (verb.toLowerCase()) {
      ...
      case 'tables': {
        ...
        await this._printResult(formatTable(result, this._tableOpts()));
        break;
      }
          ...
          try {
            ...
            if (isSelect) {
              ...
              await this._printResult(
                formatTable(result, this._tableOpts())
              );
              // ────────────────────────────────────────────────────────────────
            } else {
              const result = await this.conn.execute(sql);
