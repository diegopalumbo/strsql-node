...
function pagerName(pager) {
  const command = pagerCommandSpec(pager).command;
  return command ? command.split(/[\\/]/).pop().toLowerCase() : '';
}
...
function pagerCommandSpec(pager) {
  const value = String(pager || '').trim();
...
function envPagerCommand(pagerEnv) {
  return pagerCommandSpec(pagerEnv).command;
}
...
function shouldUsePager(content, opts = {}) {
  ...
  const lineCount = typeof content === 'number' ? content : countLines(String(content || ''));
  if (lineCount >= terminalRows() - 2) {
    return true;
  }

  const lineWidth = typeof content === 'string' ? maxLineWidth(content) : 0;
  return lineWidth > terminalColumns();
}
...
async function printWithPager(text, opts = {}) {
  ...
  // 2. Spawn pager on a temp file with inherited stdio so it can own stdin.
  // This prevents REPL keystrokes from leaking while the pager is open.
  return new Promise((resolve) => {
    const args = [...spec.args, ...pagerArgs(spec.command)];
